...
# class FeatureExtract_multicore():
#     def __init__(self, config, image_size, printout=False):
#         self.config = config
#         self.image_size = image_size
#         self.printout = printout
...
#     @ Hough Feature
#     '''
#     def hough_feature(self, image, queue):
#         # Convert to grayscale if necessary
#         if len(image.shape) > 2:
#             image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
...
#         queue.put(hough_result)
#         return hough_result

#     '''
#     @ Structure Tensor
#     '''
#     def tensor_feature(self, image, queue):
#         # Convert to grayscale if necessary
#         if len(image.shape) > 2:
#             image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
...
#     def law_feature(self, image, queue):
#         if len(image.shape) == 3:
#             # Convert to YCrCb colorspace
#             image = cv2.cvtColor(image, cv2.COLOR_BGR2YCrCb)
...
#         queue.put(law_result)
#         return law_result

#     '''
#     @ Optical Flow
#     '''
#     def flow_feature(self, image_next, image_prvs, queue):
#         # Convert to grayscale
#         image_next = cv2.cvtColor(image_next, cv2.COLOR_BGR2GRAY)
#         image_prvs = cv2.cvtColor(image_prvs, cv2.COLOR_BGR2GRAY)
...
#     @ Depth feature
#     '''
#     def depth_feature(self, image, reverse=True, queue):
#         # Reverse the pixel value
#         if reverse:
#             image = 255 - image
...
#                     if name is 'optical_flow':
#                         cropped_image_prvs = self.image_prvs[i:i+self.H_size, j:j+self.W_size]
#                         jobs.append(mp.Process(target=function, args=(cropped_image,cropped_image_prvs, queue)))
#                     else:
#                         jobs.append(mp.Process(target=function, args=(cropped_image,queue)))
#                     jobs.append(task)
