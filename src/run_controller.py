...
from controller import ExpertCtrl, LGCtrl, NNCtrl
...
controller2 = LGCtrl(client=client,
                    forward_speed=forward_speed, 
                    height=height, 
                    max_yawRate=max_yawRate,
                    image_size = (480, 640),
                    weight_file_path=os.path.join(setup_path.parent_dir, 'my_outputs/nh/iter0/ling_weight_result.csv')
                )

controller3 = NNCtrl(client=client,
                    forward_speed=forward_speed,
                    height=height,
                    max_yawRate=max_yawRate,
                    vae_model_path=os.path.join(setup_path.parent_dir, 'my_outputs/nh/iter0/VAE_checkpoint_z_10.pt'),
                    nn_model_path=os.path.join(setup_path.parent_dir, 'my_outputs/nh/iter0/NN_checkpoint_z_10.pt'),
                )
...
try:
    ...
    while True:
        ...
        controller2.set_current_yaw(state_machine.get_yawRad())
        cmd = controller2.predict(pngImg_color, pngImg_depth)
        controller2.step(cmd, state_machine.get_flight_mode())
